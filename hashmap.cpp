...
void hashmap::put(int key, int value) {
  ...
  int check;
  // checking if the hash_key exist
  if (buckets[hash_key % nBuckets] != nullptr) {
    key_val_pair *current = buckets[hash_key % nBuckets];
    ...
    // if hash_key and key not found create a new node
    // point the previous node to it
    if (check == 1) {
      nElems++;
  ...
  // if the hash_key does not exisit then create a new one with a new node
  else {
    nElems++;
    key_val_pair *Node = new key_val_pair;
    Node->value = value;
    Node->key = key;
    Node->next = nullptr;
    buckets[hash_key % nBuckets] = Node;
  }
}
...
int hashmap::get(int key) const {
  int hash_return = hashFunction(key);
  // getting the bucket associated with the key
  key_val_pair *current = buckets[hash_return % nBuckets];

  // if the bucket exisit then check each node associated
  while (current != nullptr) {
  ...
  // if the bucket is empty throw error
  if (current == nullptr) {
    throw("Key does not Exisit");
  }
  return 0;
}
...
vector<int> hashmap::keys() const {
  ...
  for (int i = 0; i < nBuckets; i++) {
    // make a node for each bucket iteration
